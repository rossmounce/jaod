...
We can retrieve who publishes it, when the DOAJ record for it was last updated, and other bibliographic information relating to the journal:

```{r}
journal <- jaod_journal(id = bibliography$result$id[1])
```

So for instance looking at the subject `journal$bibjson$subject` we can see that
